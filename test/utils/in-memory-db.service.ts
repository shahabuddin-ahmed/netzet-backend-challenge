...
export class InMemoryDbService implements IDbEntityService {
    ...
    async findAuthors(options: IAuthorQueryOptions = {}): Promise<IPaginationResult<Author>> {
        ...
        if (options.firstName) {
            ...
            filtered = filtered.filter((author) => author.firstName.toLowerCase().includes(search));
        }

        if (options.lastName) {
            const search = options.lastName.toLowerCase();
            filtered = filtered.filter((author) => author.lastName.toLowerCase().includes(search));
        }
    ...
    async findAuthorById(id: number, withBooks = false): Promise<Author | null> {
        ...
        return withBooks ? this.cloneAuthorWithBooks(author) : this.cloneAuthor(author);
    }
    ...
    async createBook(payload: ICreateBookPayload): Promise<Book> {
        const author = this.authors.find((existing) => existing.id === payload.authorId);
    ...
    async findBooks(options: IBookQueryOptions = {}): Promise<IPaginationResult<Book>> {
        ...
        if (options.isbn) {
            ...
            filtered = filtered.filter((book) => book.isbn.toLowerCase().includes(search));
        }

        if (typeof options.authorId === 'number') {
            filtered = filtered.filter((book) => book.authorId === options.authorId);
        }

        const start = (page - 1) * limit;
        const data = filtered.slice(start, start + limit).map((book) => this.cloneBook(book));
    ...
    async updateBook(id: number, payload: IUpdateBookPayload): Promise<Book> {
        ...
        if (typeof payload.authorId === 'number' && payload.authorId !== book.authorId) {
            const author = this.authors.find((existing) => existing.id === payload.authorId);
            if (!author) {
                throw new BadRequestException('Author does not exist');
            }
    ...
    private cloneAuthorWithBooks(author: Author): Author {
        ...
        cloned.books = this.books.filter((book) => book.authorId === author.id).map((book) => this.cloneBook(book));
        return cloned;
    }
