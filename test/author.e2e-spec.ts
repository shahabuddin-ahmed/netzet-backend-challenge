...
describe('Author API (e2e)', () => {
    ...
    it('creates, fetches, and filters authors', async () => {
        ...
        const createResponse = await request(app.getHttpServer()).post('/api/v1/authors').send(payload).expect(201);
        ...
        const getResponse = await request(app.getHttpServer()).get(`/api/v1/authors/${authorId}`).expect(200);
